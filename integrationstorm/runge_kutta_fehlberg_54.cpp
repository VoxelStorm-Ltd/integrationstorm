...
/*
template<typename T>
void runge_kutta_fehlberg_54<T>::integrate(vec3<T> &position,
                                           vec3<T> &velocity,
                                           T time,
                                           T delta_time) const {
  ...
  vec3<T> const temp1 = this->get_acceleration(position,
                                                  velocity,                   finaltime) * delta_time;
  vec3<T> const temp2 = this->get_acceleration(position + temp1 * c21,
                                                  velocity + c20 * delta_time, finaltime) * delta_time;
  vec3<T> const temp3 = this->get_acceleration(position + temp1 * c31
                                                           + temp2 * c32,
                                                  velocity + c30 * delta_time, finaltime) * delta_time;
  vec3<T> const temp4 = this->get_acceleration(position + temp1 * c41
                                                           + temp2 * c42
                                                           + temp3 * c43,
                                                  velocity + c40 * delta_time, finaltime) * delta_time;
  vec3<T> const temp5 = this->get_acceleration(position + temp1 * c51
                                                           + temp2 * c52
                                                           + temp3 * c53
                                                           + temp4 * c54,
                                                  velocity + c50 * delta_time, finaltime) * delta_time;
  vec3<T> const temp6 = this->get_acceleration(position + temp1 * c61
                                                           + temp2 * c62
                                                           + temp3 * c63
                                                           + temp4 * c64
                                                           + temp5 * c65,
  ...
  vec3<T> x4 = position + (temp1 * a1) + (temp3 * a3) + (temp4 * a4) + (temp5 * a5);
  vec3<T> dx =            (temp1 * b1) + (temp3 * b3) + (temp4 * b4) + (temp5 * b5) + (temp6 * b6);
  // calculate the error in 3 dimensions
  vec3<T> epsilon(std::fabs(position.x - x4.x),
